...
const ShopScreen: React.FC<ShopScreenProps> = ({ totalCoins, unlockedSkins, onBuy, onBack }) => {
  ...
  return (
    <div className="flex flex-col items-center min-h-[400px] w-full max-w-[800px] bg-background relative overflow-hidden p-4">
      ...
      <h2 className="text-lg font-pixel text-accent glow-gold mb-2">SHOP</h2>
      <p className="text-sm font-pixel text-accent mb-4">🪙 {totalCoins}</p>

      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 z-20 mb-4">
        {premiumSkins.map((skin) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[7px] font-pixel text-foreground mb-1 text-center">{skin.name}</span>
              {owned ? (
                <span className="text-[7px] font-pixel text-primary">OWNED</span>
              ) : (
                <button
                  onClick={() => {
                    if (canAfford) {
                  ...
                  disabled={!canAfford}
                  className={`px-2 py-1 text-[7px] font-pixel border transition-all ${
                    canAfford
